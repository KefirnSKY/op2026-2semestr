...
{ опирует строку из F1 в F2 без RESET или REWRITE; таким образом F1 должен быть готов дл€ чтени€, 
а F2 дл€ записи, но прошлые строки у этих файлов могут быть не пусты}
VAR
  Ch: CHAR;
BEGIN {CopyOpen}
  WHILE NOT EOLN(F1)
...
PROCEDURE AddQ (VAR Elt: CHAR); {Q = <,x/,R>,где x строка » Elt = a --> Q = <,xa/,R>}
BEGIN {AddQ}
  REWRITE(Temp);
  CopyOpen(Q, Temp);
  WRITE(Temp, Elt);
